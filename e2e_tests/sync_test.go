...
func TestSync(t *testing.T) {
	...
	// After syncing stack-2 onto stack-1 (before syncing stack-3), our commit
	// graph looks like:
	//      stack-1: main -> 1a -> 1b
	//      stack-2:                 \ -> 2a'
	//      stack-3:          \ -> 2a -> 3a
	...
	require.Contains(
		t, syncConflict.Stdout, "av sync --continue",
		"stack sync should print a message with instructions to continue",
	)
	syncContinueWithoutResolving := Av(t, "sync", "--continue")
	require.NotEqual(
		t,
		0,
...
func TestSyncWithLotsOfConflicts(t *testing.T) {
	server := RunMockGitHubServer(t)
	defer server.Close()
	repo := gittest.NewTempRepoWithGitHubServer(t, server.URL)
	Chdir(t, repo.RepoDir)
